...
public final class CameraManager {
    ...
    public CameraManager(Context context, @Nullable CameraConfigureCallback cameraConfigureCallback) {
        this.context = context;
        this.configManager = new CameraConfigurationManager(context, cameraConfigureCallback);
    ...
    public synchronized void startPreview() {
        ...
        if (theCamera != null && !previewing) {
            ...
            previewing = true;
            cameraFocusManager = new CameraFocusManager(theCamera.getCamera());
        }
    }
    ...
    /**
     * @param isOpen if {@code true}, light should be turned on if currently off. And vice versa.
     */
    public synchronized void setTorch(boolean isOpen) {
        OpenCamera theCamera = camera;
        if (theCamera != null && isOpen != configManager.getTorchState(theCamera.getCamera())) {
            boolean wasAutoFocusManager = cameraFocusManager != null;
            if (wasAutoFocusManager) {
                cameraFocusManager.stop();
                cameraFocusManager = null;
            }
            configManager.setTorch(theCamera.getCamera(), isOpen);
            if (wasAutoFocusManager) {
                cameraFocusManager = new CameraFocusManager(theCamera.getCamera());
    ...
    public synchronized void setZoom(float zoom) {
        ...
        try {
            ...
            if (parameters.isZoomSupported()) {
                ...
                int zoomTo = (int) (1.0 * maxZoom / 2 * zoom);
                if (zoomTo > maxZoom) {
                    zoomTo = maxZoom;
                } else if (zoomTo < 0) {
                    zoomTo = 0;
    ...
    public void onShotAutoFocus() {
        if (cameraFocusManager != null) {
            cameraFocusManager.onShotAutoFocus();
        }
    }
}
