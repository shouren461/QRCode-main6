...
public final class RGBLuminanceSource extends LuminanceSource {
  ...
  public LuminanceSource rotateCounterClockwise() {
    byte[] rotatedData = new byte[luminances.length];
    for (int y = 0; y < dataHeight; y++) {
      for (int x = 0; x < dataWidth; x++)
        rotatedData[x * dataHeight + dataHeight - y - 1] = luminances[x + y * dataWidth];
    }
    return new RGBLuminanceSource(rotatedData,
            dataHeight,
            dataWidth,
            top,
            left,
