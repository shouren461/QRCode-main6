...
public final class PermissionHelper {
     ...
     * 添加权限组
     */
    public PermissionHelper permission(String[] permissions) {
        return permission(PermissionUtils.asArrayList(permissions));
    }

    public PermissionHelper permission(List<String> permissions) {
        if (mPermissions == null) {
    ...
    public void request(OnPermissionCallback callback) {
        ...
        if (debugMode) {
            // 检查申请的存储权限是否符合规范
            PermissionChecker.checkStoragePermission(mContext, mPermissions, true);
            // 检查申请的定位权限是否符合规范
