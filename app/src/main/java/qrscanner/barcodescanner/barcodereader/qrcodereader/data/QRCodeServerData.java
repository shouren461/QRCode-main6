...
/**
 * 服务器配置数据管理类
 * 负责保存从 Firebase Remote Config 或其他服务器接口获取的开关、数值等配置信息
 */
public class QRCodeServerData {
     ...
     * 是否显示设置页面的评分入口
     */
    private static String isShowSettingRate = "0";
    private static String useAmazonApi = "1"; // 是否启用亚马逊商品查询 API
    private static String isNewResultUi_41 = ""; // 4.1 版本是否启用新的扫描结果页 UI
    private static String isRemoveScanBannerBG_41 = ""; // 4.1 版本是否移除结果页 Banner 广告的背景

    private static String isMainBannerShow = "1"; // 历史、设置、创建页面的 Banner 广告是否显示

    /**
     * 更新全局服务器配置（通常在应用启动时同步）
     */
    public static void ifNeedUpdate(Context context) {
        // ... 此处逻辑通常从外部库或 Firebase 获取最新值 ...
        isShowSettingRate = getRemoteConfig(context, "is_show_setting_rate_36", isShowSettingRate);
        useAmazonApi = getRemoteConfig(context, "use_amazon_api", useAmazonApi);
     ...
     * 设置页是否显示评分按钮
     */
    public static boolean isShowSettingRate() {
        return "1".equals(isShowSettingRate);
    }

    /**
     * 是否根据云端配置切换到了新的结果页 UI
     */
    public static boolean newResultUI() {
        if (TextUtils.isEmpty(isNewResultUi_41)) {
            return LocaleUtil.isGermany(); // 默认：如果是德国地区则开启
        } else {
            return "1".equals(isNewResultUi_41);
        }
    }

    /**
     * 云端配置读取封装（此处逻辑目前为占位，需配合具体下发方案实现）
     */
    private static String getRemoteConfig(final Context context, final String key, final String defaultValue) {
        String values = ""; // 此处应调用 FirebaseRemoteConfig.getInstance().getString(key)
