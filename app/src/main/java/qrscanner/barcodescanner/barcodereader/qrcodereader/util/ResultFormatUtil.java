...
//扫描结果格式化工具类 ->负责从原始的二维码/条形文本中提取最适合展示给用户看的内容(如日历标题,YouTube,搜索词等)
public class ResultFormatUtil {

    //针对扫描结果:从二维码详情例提取精简的展示信息 @param result 原始识别结果 @param handler 解析后的格式处理类型
    ...
    //针对创建结果:从生成的二维码详情里提取展示信息
    public static String extractCreateDisplayText(Result result) {
        if (result == null) {
            return "";
        }
        //注意:此处的switch使用的是BarCodeFormat的自定义扩展逻辑
    ...
    //解析iCalendar协议文本,尝试提取"标题" 或"描述"，逻辑:优先查找SUMMARY 字段，如无则找DESCRIPTION,最后保底提取开始日期
    public static String extractCalendar(String str) {
        try {
            //尝试提取标题 (SUMMARY)
            String[] s1 = str.split("DTSTART");
            if (s1.length == 2) {
                // 在 DTSTART之前的内容中查找 SUMMARY:之后的内容
    ...
    //针对YouTube链接进行精简提取  ->逻辑:去除协议头和前缀，仅保留搜索关键词，频道ID 或视频ID
