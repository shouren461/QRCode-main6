 ...
 */
class ViberHandler(activity: Activity, val parsedModel: ParseViberModel,resultHandlerConfig: ResultHandlerConfig) : BaseResultHandler(activity, parsedModel,resultHandlerConfig) {
    ...
    override fun handleButtonPress(action: ParseAction) {
        when (action) {
            ParseAction.OPEN -> {
                HandlerUtils.openViber(activity, parsedModel.uri)
            }
