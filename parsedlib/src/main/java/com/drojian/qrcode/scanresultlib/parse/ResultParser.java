 ...
 * a URL, or an e-mail address. {@link #parseResult(ScanResultModel)} will turn a raw
...
public abstract class ResultParser {
    ...
    private static final ResultParser[] PARSERS = {
            ...
            new FacebookResultParser(),
            new InstagramResultParser(),
            new WhatsAppResultParser(),
            new YoutubeResultParser(),
            new TwitterResultParser(),
            new SpotifyResultParser(),
            new PaypalResultParser(),
            new ViberResultParser(),
    ...
    protected static String getMassagedText(ScanResultModel result) {
    ...
    public static BaseParseModel parseResult(ScanResultModel theResult) {
        for (ResultParser parser : PARSERS) {
            BaseParseModel result = parser.parse(theResult);
            if (result != null) {
                return result;
            }
        }
        if (isBarCodeFormat(theResult.getCodeFormat())) {
            return new ParseBarCodeModel(theResult.getText());
        } else {
            return new ParseTextModel(theResult.getText());
        }
    }

    public static boolean isBarCodeFormat(CodeFormat codeFormat) {
        switch (codeFormat) {
            case CODE_39:
            case CODE_93:
            case CODE_128:
    ...
    /**
     * Attempts to parse the raw {@link ScanResultModel}'s contents as a particular type
     * of information (email, URL, etc.) and return a {@link BaseParseModel} encapsulating
     * the result of parsing.
     *
     * @param theResult the raw {@link ScanResultModel} to parse
     * @return {@link BaseParseModel} encapsulating the parsing result
     */
    public abstract BaseParseModel parse(ScanResultModel theResult);
